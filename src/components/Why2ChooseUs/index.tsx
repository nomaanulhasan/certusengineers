'use client';

import Image from 'next/image';
import { Swiper, SwiperSlide } from 'swiper/react';

const WHY_CHOOSE_US_DATA = [
  {
    id: 1,
    title: "In-house company",
    desc: "Students will get direct benefits of ongoing projects",
    icon: "/images/enterprise.png",
  },
  {
    id: 2,
    title: "Interview Preparation",
    desc: "We will help you to practice your interview technique.",
    icon: "/images/interview.png",
  },
  {
    id: 3,
    title: "100% Job Assistance",
    desc: "We server 100% job oriented courses with end to end placement assistance",
    icon: "/images/businessman.png",
  },
];

export default function Why2ChooseUs() {
  return (
    <div className="flex justify-center p-20">
      <div className="w-full max-w-tv">
        <h4 className="text-center font-semibold uppercase leading-loose text-section-head">
          Why to choose us?
        </h4>
        <h3 className="text-center text-3xl font-medium leading-loose text-primary-text">
          Training Features
        </h3>

        <div className="mt-8 flex gap-8 w-full">
          {WHY_CHOOSE_US_DATA.map(({ id, title, desc, icon }) => (
            <div key={id} 
                className='
                  flex flex-col justify-center flex-1 gap-6 rounded-md border p-6 group cursor-default
                  duration-300 bg-[#F1F1F2FC] hover:bg-white hover:shadow-lg items-center
                '
              >
                <div
                  className='
                    relative rounded-full bg-white group-hover:bg-[#F1F1F2FC] duration-300 w-[100px] h-[100px] flex items-center justify-center
                  '
                >
                  <Image src={icon} alt={title} width={70} height={70} />
                </div>
                <div className="flex flex-col gap-2 text-center">
                  <h5 className="font-medium text-card-head">{title}</h5>
                  <p className="text-secondary-text group-hover:text-main-orange duration-300">{desc}</p>
                </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}
